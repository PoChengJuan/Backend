...
public interface ShopDataRepository extends CrudRepository<ShopData, Integer>{
	...
	String getExpense(String shopname, String branch);
	
	@Query(value="SELECT Date,Income FROM ShopData WHERE shopname LIKE ?1 AND Branch LIKE ?2 AND Date BETWEEN ?3 AND ?4",nativeQuery = true)
